...
import { DistributionDto } from "../dtos/all-dto";
...
@ApiTags("Distribution Query")
@UseGuards(DistributionAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("distributions/query")
export class DistributionQueryController {
  #logger = new Logger(DistributionQueryController.name);

  constructor(private readonly service: DistributionQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all distribution with optional pagination" })
  @ApiResponse({ status: 200, type: DistributionsResponse })
  @ApiQuery({ name: "options", required: false, type: DistributionDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<Distribution>    
  ): Promise<DistributionsResponse<Distribution>> {
    try {
     
      const distributions = await this.service.findAll(options);
      logger.info("Retrieving all distribution");
  ...
  @ApiOperation({ summary: "Find distribution by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter distribution', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: DistributionsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Distribution no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find distributions with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<Distribution>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all distributions" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count distributions with conditions" })
  @ApiResponse({ status: 200, type: DistributionsResponse<Distribution> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one distribution with conditions" })
  ...
  @ApiOperation({ summary: "Find one distribution or return error" })
  @ApiResponse({ status: 200, type: DistributionResponse<Distribution> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @ApiParam({ name: 'id', required: true, description: 'ID of the distribution to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<DistributionResponse<Distribution>> {
    try {
      const distribution = await this.service.findOne({ where: { id } });
      if (!distribution) {
        throw new NotFoundException(
