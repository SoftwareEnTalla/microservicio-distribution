...
import { logger } from '@core/logs/logger';
  ...
  export class DistributionRepository {
    ...
    async findAll(options?: FindManyOptions<Distribution>): Promise<Distribution[]> {
      logger.info('Ready to findAll Distribution on repository:',options);
    ...
    async findById(id: string): Promise<Distribution | null> {
      const tmp: FindOptionsWhere<Distribution> = { id } as FindOptionsWhere<Distribution>;
      logger.info('Ready to findById Distribution on repository:',tmp);
    ...
    async findByField(
      ...
      logger.info('Ready to findByField Distribution on repository:',options);
    ...
    async count(): Promise<number> {
      logger.info('Ready to count Distribution on repository...');
      let result= this.repository.count();
      logger.info('Was counted  instances of Distribution on repository:');
      return result;
    }
    ...
    async findAndCount(where?: Record<string, any>): Promise<[Distribution[], number]> {
      logger.info('Ready to findByField Distribution on repository:',where);
    ...
    async findOne(options?: Record<string, any>): Promise<Distribution | null> {
      if (!options || Object.keys(options).length === 0) {
        logger.warn('No conditions provided for finding Distribution.');
        return null;
      }
      ...
      if (isFindOneOptions) {
        logger.info('Ready to findOne (FindOneOptions) Distribution:', options);
        return this.repository.findOne(options as any);
      }
      const tmp: FindOptionsWhere<Distribution> = options as FindOptionsWhere<Distribution>;
      logger.info('Ready to findOneBy Distribution on repository:', tmp);
      return this.repository.findOneBy(tmp);
    }
    ...
    async findOneOrFail(where?: Record<string, any>): Promise<Distribution> {
      logger.info('Ready to findOneOrFail Distribution on repository:',where);
    ...
    async update(
        ...
        logger.info('Updated instance of Distribution with id:  was finded on repository:', instance);
