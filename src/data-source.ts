...
export const AppDataSource = new DataSource({
  ...
  database: process.env.DB_NAME || "distribution-service",
...
async function checkPostgreSQLExtensions() {
  ...
  const distribution = await pool.connect();

  try {
    for (const ext of REQUIRED_EXTENSIONS) {
      const res = await distribution.query(
      ...
      if (res.rows.length === 0) {
        ...
        await distribution.query('CREATE EXTENSION IF NOT EXISTS "' + ext + '"');
      }
    }
  } finally {
    await distribution.release();
    await pool.end();
  }
}
